...
void turnCrank(){
    ...
    float cds_last;             //keeps track of last CdS value
...
void goToSalt(){
    ...
    float atSalt[3] = {SALT_X, SALT_Y, SALT_HEADING};
    garmin.check_any(atSalt);
    Sleep(1000);
} //goToSalt
...
void RPSChecker::check_any(float destination[]){
  ...
  if(to_heading >= 180){ to_heading -= 180;} else{ to_heading += 180;}   //flip the heading to the back ofthe robot while keeping it under 360
