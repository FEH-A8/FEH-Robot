...
struct coordinate {
  ...
  float y;
  void zero(){ x = 0; y = 0; }
  void init(float ex, float why){ x = ex; y = why; }
}
...
class Boundary {
  ...
  public:
    Boundary (coordinate first, coordinate second){ tail = first; head = second; }	//constructor
    ...
    int compareto_slab(float); 		//"slab" is a boundary line extending in the positive direction with spacing, width. Returns -1 if < both lines, 1 if > both lines, 0 if on or between lines
...
int Boundary::compareto_slab(coordinate pt, float width){
  ...
  coordinate second_tail;
  second_tail.x = x_prime;
  second_tail.y = y_prime;
  ...
  if (pt.y >= (slope * pt.x) + tail.y && pt.y <= (slop * pt.x) + second_tail.y){ 	//if y = mx + b, the point is on the boundary line
    return(0);
  }
...
coordinate intersection(Boundary * crossing){
  ...
  if (compareto_bound(crossing.getHead()) * compareto_bound(crossing.getTail()) == -1 && crossing.compareto_bound(this.getHead()) * crossing.compareto_bound(this.getTail()) == -1){
    return(true);
  }
  else{
    return(false);
...
float[] RPSChecker::current_state(){
  //0 -> X, 1 -> Y, 2 -> Heading
...
void RPSChecker::check_y(float end_x){
  //Taken from lab 2
...
void RPSChecker::check_heading(float){
  //Taken from lab 2
